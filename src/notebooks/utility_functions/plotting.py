...
import sys  
glob_Nbin=64
sys.path.insert(0, 'utility_functions')
import pandas as pd
from matplotlib.ticker import AutoMinorLocator
from mpl_toolkits.axes_grid1.inset_locator import mark_inset
from Clusterer import Strawman_Clusterer as clusterer
...
def ClusterSum_vs_GenP(clusterSum, genP, label, take_log = False, ylabel="Cluster Sum", plot_offset = 5.0):
    ...
    cluster_bins = clusterer.E_binning(np.min(clusterSum),sumE_maxPlot+plot_offset, take_log)
    truth_bins   = clusterer.E_binning(np.min(genP),np.max(genP)+plot_offset, take_log)
