...
class MPGraphDataGenerator:
    def __init__(self,
        ...
        if self.num_targetFeatures==2:
            ...
            self.scalar_keys = self.nodeFeatureNames + ['ecal_energy','ecal_posz', 'ecal_posx', 'ecal_posy']+["clusterE","genP"]
    ...
    def get_cell_data(self,event_data):
        ...
        #print('hcal  ', len(cell_data_swaped),'    ', len(cell_data_ecal_swaped) , '  ecal ', len(cell_data_ecal),'    cell data hcal ', len(cell_data))
    ...
    def get_GenP_Theta(self,event_data,event_ind):
        ...
        genP = np.log10(np.sqrt(genPx*genPx + genPy*genPy + genPz*genPz))
        genP = (genP - self.means_dict["genP"]) / self.stdvs_dict["genP"]
        theta = (genP - self.means_dict["theta"]) / self.stdvs_dict["theta"]
        return genP, theta
